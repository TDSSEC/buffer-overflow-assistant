...
otherScans(){
	...
	do
	    ...
	    python3 ~/opt/dirsearch/dirsearch.py -u https://$ip -e php,asp,aspx,js | tee -a nse_scans/dirsearch-443-$ip.txt
	done
    else
	:
    fi            
...
discoveryScans(){
	...
	do
	    python3 ~/opt/dirsearch/dirsearch.py -u http://$ip -e php,asp,aspx,js | tee -a nse_scans/dirsearch-80-$ip.txt
	done 
    else
	:
    fi
